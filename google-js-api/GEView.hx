...
*/
public static function project( lat : Float, lng : Float, alt : Float, altitudeMode : KmlAltitudeModeEnum ) : KmlVec2;

/*
Sets the  camera that views the scene in Google Earth.
*/
public static function setAbstractView( view : KmlAbstractView ) : Void;
...
GEPlugin.ALTITUDE_RELATIVE_TO_SEA_FLOOR
*/
public static function copyAsLookAt( altitudeMode : KmlAltitudeModeEnum ) : KmlLookAt;
...
GEPlugin.ALTITUDE_RELATIVE_TO_SEA_FLOOR
*/
public static function copyAsCamera( altitudeMode : KmlAltitudeModeEnum ) : KmlCamera;
...
*/
public static function getViewportGlobeBounds() : KmlLatLonBox;
...
Given a point on the screen in pixel coordinates, returns a GEHitTestResult with information about the geographic location corresponding to the point on the screen. Tip: hitTest() is the inverse of project(). Parameters:
  x The x coordinate, measured from the left or right edge of the plug-in window, depending on xUnits. xUnits The units in which the x value is specified. y The y coordinate, measured from the top or bottom edge of the plug-in window, depending on yUnits. yUnits The units in which the y value is specified. mode Selects which categories of objects to include in the hit test. Categories may be combined via JavaScript's OR operator, '|'. Returns:Returns a GEHitTestResult, or null if the screen point is not over anything selected by mode. Note that for HIT_TEST_GLOBE, resulting altitude values will always be 0. See also:GEPlugin.UNITS_PIXELS 
...
*/
public static function hitTest( x : Float, xUnits : KmlUnitsEnum, y : Float, yUnits : KmlUnitsEnum, mode : GEHitTestModeEnum ) : GEHitTestResult;
...
*/
public static function viewchangebegin() : Void;

/*
Event fired when the view stops changing in Earth.
*/
public static function viewchangeend() : Void;
...
*/
public static function viewchange() : Void;
